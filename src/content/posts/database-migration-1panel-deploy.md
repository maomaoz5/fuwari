...
就这么简单。启动项目后，数据库文件会自动创建在 `data/stats.db`。首次启动时会自动建表并创建默认管理员账号（`admin` / `admin12345678`）。
...
通过 `.env` 中的 `CREATE_DEFAULT_ADMIN` 配置项控制是否自动创建默认管理员账号：

```env
# 设置为 true 时，首次启动自动创建默认管理员（默认值：true）
CREATE_DEFAULT_ADMIN=true
```

如果你不需要默认管理员（例如已经手动创建了其他管理员），可以将其设为 `false`，这样启动时不会自动创建 `admin` 账号。

#### 忘记密码：邮箱重置

登录页面新增了「忘记密码」功能。管理员在登录页点击「忘记密码」后，系统会通过 Resend 邮件服务向管理员绑定的邮箱发送密码重置链接。

- 管理员可在管理面板中设置自己的邮箱地址
- 重置链接通过邮件发送，点击后即可设置新密码
- 需要在 `.env` 中配置 Resend API Key 和发件人信息

#### CLI 密码重置

如果邮件服务不可用，也可以通过命令行脚本直接重置密码：

```bash
node scripts/reset-admin-password.mjs
```

该脚本同时支持 SQLite 和 PostgreSQL 两种数据库模式，会根据 `DB_TYPE` 环境变量自动选择对应的数据库驱动。
